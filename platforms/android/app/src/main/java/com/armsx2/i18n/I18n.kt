...
private val BASE_EN: Map<String, String> = mapOf(
    ...
    "perf.eeCycleRate.description" to "CPU overclock/underclock. Higher can help CPU-bound games but may break timing. Dynamic underclocks on its own when the device falls behind, and never goes above 100%. A game whose database entry sets a cycle rate, or one with a per-game cycle rate of its own, keeps that fixed rate even on Dynamic.",
    "perf.eeCycleRate.dynamic" to "Dynamic",
