...
// Host-MMU fastmem backpatch. Called from vtlb_BackpatchLoadStore
// (vtlb.cpp) when a single fastmem Ldr/Str at `code_address` faults on a
// handler/MMIO/unmapped page. Carves a thunk that redoes the access via the slow
// vtlb_mem* helper (using the recorded address/data registers), resumes at the
// instruction after the fault, then overwrites the faulting Ldr/Str with a branch
// to the thunk so subsequent hits skip straight to the slow path. Our cycle model
// is memory-based, so there is no cycle flush/reload.
//
// address_register / data_register are ARM64 register codes; they can be EE
// guest-GPR cache regs (x20/x22-x27), not just scratch, so the moves preserve
// them. RESTATEPTR(x19)/REVTLBPTR(x21)/RFASTMEMBASE(x28) are callee-saved and
// survive the C call. is_fpr is always false here: LWC1/SWC1 stage the FP value
// through a GPR, so their memory access is a plain 32-bit integer access.
// size_in_bits==128 is the LQ/SQ vector path.
void vtlb_DynBackpatchLoadStore(uptr code_address, u32 code_size, u32 guest_pc, u32 guest_addr, u32 gpr_bitmask, u32 fpr_bitmask, u8 address_register, u8 data_register, u8 size_in_bits, bool is_signed, bool is_load, bool is_fpr)
{
	...
	if (size_in_bits == 128)
	{
		// 128-bit quad via vtlb_memRead128/vtlb_memWrite128 (r128 value in q0 = RQRET).
		// data is a V-register (banked separately from the GPR address), so no aliasing.
		if (is_load)
		{
	...
	else if (is_load)
	{
		...
		// Extend the sub-word return into the FULL x0 (the C ABI leaves the high bits
		// of a sub-word return undefined) so the X-move below carries the correct value
		// — matching the interpreter's load semantics and the direct fastmem load.
		switch (size_in_bits)
		{
	...
	{
		// vtlb_memWrite<T>(u32 addr -> w0, T value -> x1/w1). Resolve aliasing where
		// addr sits in x1 and data in x0 (moving one would clobber the other).
