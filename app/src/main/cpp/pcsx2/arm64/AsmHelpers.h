...
// In-place patch: overwrite the 4-byte B at `code_address` with a branch to `target`.
// Used by the fastmem backpatch to redirect a faulting Ldr/Str to its slow-path thunk.
// `code_address` must already hold a single 4-byte instruction; target within +/-128 MB.
