...
// x28 = host-MMU fastmem base (vtlbdata.fastmem_base). Pinned in recGenDispatchers
// under CHECK_FASTMEM; dropped from the EE guest-GPR cache (8->7 slots) to free it.
// The vmap path via REVTLBPTR(x21) is the non-fastmem fallback, so x21 stays reserved.
...
// Allocate a scratch code region for fastmem backpatch thunks from
// the EE code buffer (no const pool) for the SIGSEGV backpatch thunk. Called only
// from the fault handler (vtlb_DynBackpatchLoadStore, RecStubs.cpp), never mid-block.
u8* recBeginThunk();
u8* recEndThunk();
...
// store: src). Returns true if fastmem was emitted (caller skips the vmap path).
