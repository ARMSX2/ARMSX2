	...
	: GSRenderer(), m_fzb(NULL)
{
	...
	// GSCurrentRenderer, i.e. as GSRendererTile's fallback floor.
	ResetHandlers();
}
...
void GSRendererSW::RecordDrawLogEntry() const
{
	...
	// The draw environment rather than the live one: under the split parser the front
	// object has already run ahead, so m_env is a later frame's answer to this question.
...
// The SW engine's flush rule regardless of process renderer type: GSRendererTile runs this
...
// renderer on self-texturing draws (FlatOut 2 diverged byte-for-byte in the M0 corpus run).
