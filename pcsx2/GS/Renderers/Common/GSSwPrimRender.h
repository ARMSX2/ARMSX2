...
// else. It used to be three members of GSRendererHW; it is a struct because a second renderer
// (TileGpu) now has the same road and the two must not grow separate copies of the setup.
//
...
// memory. ONE definition, called by the caller and handed back in: the pages a CPU-rasterized draw
// writes have to be decided before the draw runs (TileGpu tests them for GPU truth) and reported
