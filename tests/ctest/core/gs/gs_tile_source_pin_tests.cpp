...
// GSDevice's pool, recycle and fetch logic is real (it is not virtual), and only surface creation
// is stubbed. That means the test exercises the same Recycle-to-pool path the renderer does, which
// is the point — "a recycled texture comes back out of the pool" is half of what the pin prevents.
...
namespace
{
	...
	// Everything below CreateSurface is GSDevice's own, non-virtual: the pool, the LRU, Recycle and
	// FetchSurface all run for real.
