...
// =========================================================================================
// THE COLOUR-FREE VARIANT (EmuCore/GS/TileGpuNoRgbFragmentVariant), sized and structurally checked.
//
// A run whose pipeline colour write mask keeps every RGB channel gets a program with the texture
// function's colour half, the fog walk and the integer blend arm left out (tilegpu.glsl's
// TILEGPU_NO_RGB). Two things have to hold and neither is visible from the C++ side:
//
//   IT COMPILES. A variant that fails to compile falls back to the FULL module, silently, and the
//   full module is roughly twice the a650's instruction-size cliff -- so a broken colour-free arm
//   would be a large slowdown reported as nothing at all.
//
//   IT IS SMALLER, AND NOT BY DROPPING THE ALPHA. Smaller is the whole point; but an arm that also
//   skipped the texel fetch would be smaller AND wrong, because the alpha the test reads comes from
//   the texel wherever TCC is on. So the fetch is checked to still be there, by size: a paletted
//   colour-free program has to stand well clear of the untextured one.
// =========================================================================================
TEST(GSTileGpuShaderBudget, TheColourFreeVariantCompilesSmallerAndKeepsTheAlphaTexel)
{
	const GSTileSwizzleForms::FormSet forms = GSTileSwizzleForms::Fit();
	ASSERT_TRUE(forms.valid);
	const std::string body = ReadShaderSourceFile("tilegpu.glsl");
	...
	{
		GTEST_SKIP() << "shaderc is not loadable here, so no program can be sized. This is a SKIP, not a pass: "
						"the colour-free fragment variant was NOT checked on this machine.";
	}
	const std::string prologue = kStageHeader + GSTileSwizzleForms::ShaderDefines(forms) +
	                             GSTileGpuShaderVariant::DeviceDefines(true, true, true);

	// A spec with TCC on, which is the case that must keep sampling: the fragment alpha the test
	// reads is the texel's.
	GSDevice::GSTileGpuFragmentSpec tcc_on;
	tcc_on.valid = true;
	tcc_on.tfx = 0; // MODULATE
	tcc_on.tcc = 1;
	tcc_on.atst = ATST_GEQUAL;
	GSDevice::GSTileGpuFragmentSpec unfrozen; // reads every axis off the row
	...
	const Case cases[] = {
		{"untextured", 0, 0, 0, false, &unfrozen},
		{"untextured q16", 0, 0, 0, true, &unfrozen},
		{"byte[IDX8] spec tcc=1", GSDevice::kGSTileGpuRoadByte, GSDevice::kGSTileGpuTexelIndex8, 0, false, &tcc_on},
		{"byte[IDX8] unfrozen", GSDevice::kGSTileGpuRoadByte, GSDevice::kGSTileGpuTexelIndex8, 0, false, &unfrozen},
		{"target", GSDevice::kGSTileGpuRoadTarget, 0, 0, false, &unfrozen},
		{"target self{all} q16", GSDevice::kGSTileGpuRoadTarget, 0, GSDevice::kGSTileGpuSelfMaskAll, true, &unfrozen},
		{"source self{blend}", GSDevice::kGSTileGpuRoadSource, 0, GSDevice::kGSTileGpuSelfBlend, false, &unfrozen},
	};

	u32 untextured_no_rgb = 0, paletted_no_rgb = 0;
	std::printf("\nTileGpu colour-free fragment variant (SPIR-V words)\n");
	std::printf("  %-28s %9s %9s %8s\n", "variant", "full", "no-rgb", "saved");
	for (const Case& c : cases)
	{
		std::string err_full, err_lean;
		const std::string spec_defines = GSTileGpuShaderVariant::SpecDefines(*c.spec);
		const u32 full = sc.FragmentWords(prologue +
											  GSTileGpuShaderVariant::VariantDefines(
												  c.road, c.texel, c.self, c.quantise, /*no_rgb=*/false) +
											  spec_defines + body,
			err_full);
		const u32 lean = sc.FragmentWords(prologue +
											  GSTileGpuShaderVariant::VariantDefines(
												  c.road, c.texel, c.self, c.quantise, /*no_rgb=*/true) +
											  spec_defines + body,
			err_lean);
		ASSERT_NE(full, 0u) << c.name << " (full) failed to compile: " << err_full;
		ASSERT_NE(lean, 0u) << c.name << " (no-rgb) failed to compile: " << err_lean
							<< " -- a colour-free variant that will not compile falls back to the FULL module, "
							   "which is about twice the a650's size cliff, and says nothing.";
		std::printf("  %-28s %9u %9u %8d\n", c.name, full, lean, int(full) - int(lean));
		EXPECT_LT(lean, full) << c.name
							  << ": the colour-free program is not smaller than the full one, so the arm is doing "
								 "nothing -- which is the failure mode with no symptom.";
		if (c.road == 0 && !c.quantise)
			untextured_no_rgb = lean;
		if (c.road == GSDevice::kGSTileGpuRoadByte && c.spec == &tcc_on)
			paletted_no_rgb = lean;
	}

	// ⚠️ THE ALPHA FETCH IS STILL THERE. An eight-bit paletted decode is hundreds of words of address
	// arithmetic and a palette lookup; if the colour-free arm had dropped `cv.a = ct.a` along with the
	// colour, the texel would be unreferenced and the optimizer would take the whole decode with it,
	// landing this program on top of the untextured one. The margin is set well under the measured
	// distance so it is a structural check and not a fit.
	ASSERT_NE(untextured_no_rgb, 0u);
	ASSERT_NE(paletted_no_rgb, 0u);
	EXPECT_GT(paletted_no_rgb, untextured_no_rgb + 200u)
		<< "the paletted colour-free program (" << paletted_no_rgb << " words) has collapsed onto the untextured one ("
		<< untextured_no_rgb << "): its texel fetch is gone, and with it the alpha the test reads.";
}
