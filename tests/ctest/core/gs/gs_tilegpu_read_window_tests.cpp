...
TEST(GSTileGpuReadWindow, NarrowsToTheCoverageWhenTheModesAreIntervals)
{
	...
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_CLAMP, CLAMP_CLAMP);
...
TEST(GSTileGpuReadWindow, KeepsTheWholeWindowWhenTheCoverageIsTheWholeWindow)
{
	...
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), Window(), kTwLog2, kThLog2, CLAMP_REPEAT, CLAMP_REPEAT);
...
TEST(GSTileGpuReadWindow, NarrowsARepeatDrawThatStaysInsideOnePeriod)
{
	...
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_REPEAT, CLAMP_REPEAT);
	EXPECT_TRUE(r.eq(Slack(cov, Window())));
	EXPECT_TRUE(CoversX(r, lo, hi));
...
TEST(GSTileGpuReadWindow, NarrowsToTheRegionARegionClampNames)
{
	...
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_REGION_CLAMP, CLAMP_REPEAT);
	EXPECT_TRUE(r.eq(Slack(cov, Window())));
	EXPECT_TRUE(CoversX(r, lo, hi));
	EXPECT_LT(r.z, kTw);
}
...
TEST(GSTileGpuReadWindow, TheLinearSkirtIsInTheRectItIsHandedAndSurvivesTheNarrowing)
{
	...
	const GSVector4i rn = gsTileGpuReadWindowRect(Window(), nearest, kTwLog2, kThLog2, CLAMP_CLAMP, CLAMP_REPEAT);
	const GSVector4i rl = gsTileGpuReadWindowRect(Window(), linear, kTwLog2, kThLog2, CLAMP_CLAMP, CLAMP_REPEAT);
...
TEST(GSTileGpuReadWindow, RefusesRegionRepeatOnU_TheRedCheck)
{
	// REGION_REPEAT addresses (u & MINU) | MAXU. With MINU = 0x0F and MAXU = 0x40 the reachable
	// texels are 64..79 -- but ALSO nothing else, and a span that walks the whole axis produces
	// exactly that set with holes nowhere near where a coordinate box would put them. The mode's
	// rect is a hint the texture cache pairs with a preserved wrap mode; it is not a claim that
	// nothing outside it is fetched, which is why the guard refuses rather than trusts.
	...
	// The rect GetTextureMinMax hands back for this mode (vr.x = MAXU, vr.z = (MAXU | MINU) + 1).
	const GSVector4i cov(kMaxU, 0, (kMaxU | kMinU) + 1, kTh);

	// The guard: the window comes back untouched, so every page stays composed.
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_REGION_REPEAT, CLAMP_REPEAT);
	EXPECT_TRUE(r.eq(Window()));

	// THE RED CHECK -- what shipping the rect anyway would have cost. Take the guard out (which is
	// what `cov` is) and there are texels the sampler reaches that the rect does not hold, on the V
	// axis of a draw whose U mode is REGION_REPEAT: the U rect is a hull of a bit pattern, and a
	// pattern with a set bit above the hull's top is exactly what MAXU >= tw produces.
	constexpr int kBigMaxU = 0x100; // MAXU at the window's own width
	const std::vector<int> big = ReachedSet(0, 4095, CLAMP_REGION_REPEAT, kTwLog2, kMinU, kBigMaxU);
	const GSVector4i big_cov(kBigMaxU, 0, (kBigMaxU | kMinU) + 1, kTh);
	// The rect starts at 256 on a 256-wide window: intersected, it is empty, and a composed set of
	// nothing would sample every texel from the zero slot.
	EXPECT_TRUE(big_cov.rintersect(Window()).rempty());
	EXPECT_GE(big.back(), kBigMaxU);
	// The guard returns the window regardless of which of the two shapes arrives.
	EXPECT_TRUE(
		gsTileGpuReadWindowRect(Window(), big_cov, kTwLog2, kThLog2, CLAMP_REGION_REPEAT, CLAMP_REPEAT).eq(Window()));
}

TEST(GSTileGpuReadWindow, RefusesRegionRepeatOnV_TheRedCheck)
{
	// The same guard on the other axis, because a mode test written once is a mode test that
	// covers one axis. The U rect here is an honest CLAMP interval, so only WMT refuses -- and the
	// refusal has to take the whole rect with it, not just its V half: the page footprint is one
	// rectangle and half a narrowing is a narrowing.
	const GSVector4i cov(16, 0x40, 80, (0x40 | 0x0F) + 1);
	const GSVector4i r = gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_CLAMP, CLAMP_REGION_REPEAT);
	EXPECT_TRUE(r.eq(Window()));

	// THE RED CHECK: without the guard the U half alone would already have narrowed the composed
	// set to three pages of a 32-page window, while the V axis reaches texels outside [0x40, 0x4F].
	const std::vector<int> reached = ReachedSet(0, 4095, CLAMP_REGION_REPEAT, kThLog2, 0x0F, 0x40);
	EXPECT_FALSE(cov.rintersect(Window()).eq(Window())); // the rect really would have narrowed
	EXPECT_EQ(reached.front(), 0x40);
	EXPECT_EQ(reached.back(), 0x4F);
}
...
TEST(GSTileGpuReadWindow, RefusesAWindowTheRegisterSizesAbove1024_TheRedCheck)
{
	...
	EXPECT_TRUE(gsTileGpuReadWindowRect(win, cov, 12, 10, CLAMP_CLAMP, CLAMP_CLAMP).eq(win));
	EXPECT_TRUE(gsTileGpuReadWindowRect(win, cov, 10, 12, CLAMP_CLAMP, CLAMP_CLAMP).eq(win));
	...
	EXPECT_TRUE(gsTileGpuReadWindowRect(win, GSVector4i(0, 0, 64, 64), 11, 10, CLAMP_CLAMP, CLAMP_CLAMP).eq(win));
	// At TW = TH = 10 the same rect is admitted, so the guard is the size and not a blanket.
	EXPECT_TRUE(gsTileGpuReadWindowRect(win, GSVector4i(0, 0, 64, 64), 10, 10, CLAMP_CLAMP, CLAMP_CLAMP)
...
TEST(GSTileGpuReadWindow, RefusesAnEmptyIntersection_TheRedCheck)
{
	...
	EXPECT_TRUE(gsTileGpuReadWindowRect(Window(), cov, kTwLog2, kThLog2, CLAMP_CLAMP, CLAMP_CLAMP).eq(Window()));
	...
	EXPECT_TRUE(gsTileGpuReadWindowRect(Window(), GSVector4i(32, 0, 32, kTh), kTwLog2, kThLog2, CLAMP_CLAMP,
		CLAMP_CLAMP)
					.eq(Window()));
}
			...
			for (int x = -64; x <= kTw + 64; x += 37)
			{
				for (int w = 0; w <= kTw + 64; w += 53)
				{
					...
					const GSVector4i r = gsTileGpuReadWindowRect(win, cov, kTwLog2, kThLog2,
						static_cast<u32>(mode_s), static_cast<u32>(mode_t));
					...
					if (mode_s == CLAMP_REGION_REPEAT || mode_t == CLAMP_REGION_REPEAT)
						EXPECT_TRUE(r.eq(win)) << "region repeat must not narrow";
				}
			}
		}
	}
