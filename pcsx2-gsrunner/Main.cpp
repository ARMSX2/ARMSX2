...
struct DeviceWaitBill
{
	...
	u64 kicks_offered = 0;
	u64 kicks_taken = 0;
	// The same waits itemised by CALL SITE. The four bills above say what waiting cost; these say
...
static void WriteStatsJson(const std::string& path)
{
	...
	// to the four figures above exactly.
...
void GSRunner::DumpStats()
{
	...
	// disappears exactly where the waits are is worse than no split at all.
	{
		...
		static constexpr const char* kFamilies[4] = {"sync", "out-of-band", "source-set", "ring"};
		...
		Console.WriteLn(fmt::format("@HWSTAT@ GPU Wait Sites:{}   [sums: sync {:.3f} ms over {} + out-of-band "
									"{:.3f} ms over {} + source-set {:.3f} ms over {}; ring {:.3f} ms over {}]",
